...
void main() {
  ...
  for (var version = 1; version <= DatabaseHelper.schemaVersion; version++) {
    test(
      ...
      () async {
        ...
        if (version < 13) expect(orders.single['command_id'], isNull);
        expect(await upgraded.query('order_items'), hasLength(1));
        expect(await upgraded.query('debts'), hasLength(1));
        final debt = (await upgraded.query('debts')).single;
        if (version < 14) expect(debt['due_date'], isNull);
  ...
  test(
    'v12 order migration preserves rows and enforces command per user',
    () async {
      final databasePath = path.join(testDirectory.path, 'order_command.db');
      final fixture = await databaseFactoryFfi.openDatabase(databasePath);
      await _createRecentFixture(fixture, 12);
      await fixture.execute('PRAGMA user_version = 12');
      await fixture.close();
      ...
      final original = Map<String, Object?>.from(
        (await upgraded.query('orders')).single,
      );
      expect(original['command_id'], isNull);
...
Future<void> _createRecentFixture(
  ...
  if (version == 12) {
    await db.execute('DROP INDEX idx_orders_user_command_id');
    await db.execute('ALTER TABLE orders DROP COLUMN command_id');
  }
  if (version < 14) {
  ...
  await db.insert('debts', {
    ...
    if (version >= 14) 'due_date': '2026-01-08T00:00:00.000Z',
    'status': 'open',
  });
  if (duplicateDebt) {
    await db.insert('debts', {
      ...
      if (version >= 14) 'due_date': '2026-01-09T00:00:00.000Z',
      'status': 'open',
    });
  }
  await db.insert('resellers', {
...
Future<void> _expectCompleteSchema(Database db) async {
  for (final entry in {
    'orders': {'customer_pay_amount_centavos', 'command_id', 'schema_version'},
    'debts': {
  ...
  expect(
    ...
    contains(
      predicate<Map<String, Object?>>(
        (row) =>
            row['name'] == 'idx_orders_user_command_id' && row['unique'] == 1,
      ),
    ),
  );
