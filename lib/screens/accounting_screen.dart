...
class _AccountingScreenState extends State<AccountingScreen>
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      listenable: AppState(),
      builder: (context, _) {
        final allOrders = AppState().orders.toList();
        final orders    = AccountingService.instance.filterByDateRange(
            allOrders, from: _fromDate, to: _toDate);

        final gross        = AccountingService.instance.grossSales(orders);
        final discounts    = AccountingService.instance.totalDiscounts(orders);
        final net          = AccountingService.instance.netSales(orders);
        final customRev    = AccountingService.instance.customizedOrderRevenue(orders);
              ...
              child: Column(
                ...
                children: [
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(children: [
                      _SummaryCard('Gross Sales (SRP)', _currency.format(gross),
                          AppColors.whiteSecondary, Icons.bar_chart),
                      const SizedBox(width: 10),
                      _SummaryCard('Total Discounts', _currency.format(discounts),
                          AppColors.warning, Icons.discount_outlined),
                      const SizedBox(width: 10),
                      _SummaryCard('Net Sales\n(after all discounts)', _currency.format(net),
                          AppColors.gold, Icons.trending_up),
                      const SizedBox(width: 10),
                      _SummaryCard('Custom Orders', _currency.format(customRev),
                          AppColors.info, Icons.draw_outlined),
                    ]),
                  ),
                  const SizedBox(height: 12),
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  _OrderTable(
                      orders: orders
                          .where((o) => o.orderType == 'customized')
                          .toList(),
                      currency: _currency),
                ],
              ),
...
class _OrderTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (orders.isEmpty) {
      return const Center(
        child: Text('No records', style: TextStyle(color: AppColors.whiteTertiary)),
      );
    }
    ...
    return SingleChildScrollView(
      ...
      child: SingleChildScrollView(
        ...
        child: DataTable(
          ...
          rows: orders.map((o) {
            final totalDiscount = o.totalDiscountAmount;
            final hasDiscount   = totalDiscount > 0;
            // SRP = sum of (srpPrice × qty) per item — true catalog price before any discount
            final srpAmount     = o.srpTotal;
            // Disc. Price = the actual amount customer pays (net)
            final discPrice     = o.customerPayAmount;
            // Net = same as discPrice (customerPayAmount is the true net)
            final netAmount     = o.netAfterAllDiscounts;
            return DataRow(cells: [
              DataCell(Text(o.orderId,
                  style: const TextStyle(color: AppColors.gold, fontSize: 10))),
              DataCell(Text(o.customerName)),
              DataCell(Text(currency.format(srpAmount))),
              DataCell(Text(currency.format(discPrice))),
              DataCell(Text(
                hasDiscount ? '− ${currency.format(totalDiscount)}' : '—',
                style: TextStyle(
                    color: hasDiscount ? AppColors.warning : AppColors.whiteTertiary),
              )),
              DataCell(Text(
