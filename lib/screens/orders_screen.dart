...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  bool _bulkDelivering =
      false; // Guards the bulk-deliver button against double-tap
  ...
  Future<void> _bulkMarkDelivered(List<Order> filtered) async {
    final candidates = filtered
        ...
        .toList();
    final openDebtOrderIds = AppState().debts
        .where((debt) => !debt.isPaid)
        .map((debt) => debt.orderId)
        .toSet();
    final blocked = candidates
        .where((order) => openDebtOrderIds.contains(order.orderId))
        .toList();
    final deliverable = candidates
        .where((order) => !openDebtOrderIds.contains(order.orderId))
        .toList();
    if (deliverable.isEmpty) {
      final message = blocked.isEmpty
          ? 'No shipped orders are ready to mark as delivered.'
          : 'Orders with unpaid debt cannot be bulk-delivered.';
      KnzToast.warning(context, message);
      return;
    }
    final skippedMessage = blocked.isEmpty
        ? ''
        : ' ${blocked.length} order${blocked.length == 1 ? '' : 's'} with '
              'unpaid debt will be skipped.';
    final confirmed = await showConfirmDialog(
      context,
      title: 'Mark All as Delivered?',
      message:
          'This will mark ${deliverable.length} '
          'order${deliverable.length == 1 ? '' : 's'} as Delivered.'
          '$skippedMessage Continue?',
    ...
    try {
      ...
      if (mounted) {
        KnzToast.success(
          context,
          '✅ $updated order${updated == 1 ? '' : 's'} marked as Delivered.',
        );
      }
    } catch (_) {
      if (mounted) {
        KnzToast.error(
          context,
          updated == 0
              ? 'No orders were updated. Please try again.'
              : '$updated order${updated == 1 ? '' : 's'} updated before an '
                    'error stopped the bulk action.',
        );
      }
    } finally {
      if (mounted) setState(() => _bulkDelivering = false);
    }
  }
    ...
    final result = await showDialog<OrderStatus>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...OrderStateMachine.allowedTransitions(order.status).map(
              (s) => ListTile(
    ...
    try {
      if (result == OrderStatus.utang && mounted) {
        ...
        if (mounted) {
          KnzToast.success(
            context,
            '✅ ${order.orderId} → ${result.displayName}.',
          );
        }
      }
    } catch (e) {
  ...
  void _deleteOrder(Order order) async {
    final confirm = await showConfirmDialog(
      ...
      title: 'Delete Order',
      message:
          'Remove order ${order.orderId} (${order.customerName})? It can be restored from the Recycle Bin.',
      confirmLabel: 'Delete',
    ...
    try {
      ...
      if (mounted) {
        KnzToast.info(context, 'Order ${order.orderId} moved to Recycle Bin.');
      }
    } catch (error) {
      if (mounted) {
        KnzToast.error(
            ...
            (item) => Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    (item.srpPrice * item.quantity).format(),
                    style: const TextStyle(
                      color: AppColors.whiteTertiary,
                      fontSize: 12,
                    ),
