...
class ReportsScreen extends StatefulWidget {
  const ReportsScreen({super.key, this.accountingReport});
...
class _ReportsScreenState extends State<ReportsScreen> {
    ...
    return orders
        .where(
          (o) =>
              ...
              !o.orderDate.isAfter(endOfDay),
        )
        .toList();
  }

  List<CustomerDebt> _filteredDebts() {
    // Receivables are an as-of snapshot, not a debt-creation-period metric.
    return AppState().debts.toList();
  }

  AccountingReport _accountingReport() =>
  ...
  _PreviewStats _buildPreview() {
    ...
    final report = _accountingReport();
    ...
    switch (_selected) {
      ...
      case _ReportType.outstandingDebts:
        ...
        final unpaidReport = svc.summarize(orders: const [], debts: unpaid);
        return _PreviewStats(
          count: unpaid.length,
          label: 'unpaid debts',
          total: unpaidReport.receivables,
          totalLabel: 'outstanding',
        );
      case _ReportType.debtWithInterest:
        final wi = debts.where((d) => d.hasInterest && !d.isPaid).toList();
        final interestReport = svc.summarize(orders: const [], debts: wi);
        return _PreviewStats(
          count: wi.length,
          label: 'debts with interest',
          total: interestReport.receivables,
      ...
      case _ReportType.profitLoss:
        return _PreviewStats(
          count: report.recognizedOrders.length,
          label: 'orders',
          total: report.cashReceived,
  ...
  Future<void> _export({required bool asCsv}) async {
    ...
    try {
      ...
      switch (_selected) {
        case _ReportType.salesMonthly:
          asCsv
              ? await ExportService.exportOrdersCsv(recognized)
        ...
        case _ReportType.resellerSales:
          final reseller = recognized.where((o) => o.isReseller).toList();
          asCsv
        ...
        case _ReportType.outstandingDebts:
          final unpaid = filteredDebts.where((d) => !d.isPaid).toList();
          asCsv
        ...
        case _ReportType.debtWithInterest:
          final withInterest = filteredDebts
              .where((d) => d.hasInterest && !d.isPaid)
              .toList();
          asCsv
        ...
        case _ReportType.customOrderStatus:
          ...
          asCsv
              ...
              : await ExportService.exportCustomOrdersPdf(
                  state.customOrders,
                  businessName: biz,
                );
          break;
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_selected.supportsDateRange) ...[
                                ...
                                Semantics(
                                  button: true,
                                  label: 'Choose report date range',
                                  child: InkWell(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _PLRow(
            label: 'Debt Collections (cash flow)',
            value: report.debtCollections,
            color: AppColors.success,
          ),
          _PLRow(
            label: 'Custom-order receipts',
            value: report.customOrderReceipts,
            color: AppColors.info,
          ),
          const Divider(color: AppColors.divider, height: 16),
          _PLRow(
