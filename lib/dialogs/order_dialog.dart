    ...
    try {
      ...
      final interestNote =
          _paymentMethod == PaymentMethod.utang &&
              _interestType != 'none' &&
              (Money.tryParse(_interestCtrl.text.trim()) ?? Money.zero)
                  .isPositive
          ? '\nInterest: ${_interestCtrl.text.trim()}% $_interestType'
          : '';
      final dueDateNote =
          _paymentMethod == PaymentMethod.utang && _dueDate != null
          ? '\nDue: $_dueDateLabel'
          : '';
      final confirmed = await showConfirmDialog(
        ...
        title: 'Create Order?',
        message:
            'Create order for $customer with ${_cart.length} item(s) totaling ${displayTotal.format()}$resellerNote?$interestNote$dueDateNote',
      ...
      final success = await state.addOrder(
        ...
        interestType: _paymentMethod == PaymentMethod.utang
            ? _interestType
            : 'none',
        dueDate: _paymentMethod == PaymentMethod.utang ? _dueDate : null,
      );
      if (success && mounted) {
            ...
            children: [
                          ...
                          Column(
                            ...
                            children: [
                              Builder(
                                builder: (_) {
                                  final srp =
                                      _pickedProduct?.price ?? Money.zero;
                                  final deduct =
              ...
              DarkDropdown<OrderStatus>(
                ...
                onChanged: (v) {
                  ...
                  setState(() {
                    _status = v;
                    if (v == OrderStatus.utang) {
                      _paymentMethod = PaymentMethod.utang;
                    } else if (_paymentMethod == PaymentMethod.utang) {
                    ...
                    onChanged: (v) {
                      if (v != null) {
                        setState(() {
                          ...
                          if (v == PaymentMethod.utang) {
                            _status = OrderStatus.utang;
                          } else if (_status == OrderStatus.utang) {
