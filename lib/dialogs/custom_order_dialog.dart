...
class _CustomOrderDialogState extends State<CustomOrderDialog> {
  ...
  Future<void> _save() async {
    ...
    final deposit = Money.tryParse(_depositCtrl.text.trim());
    if (price == null ||
        deposit == null ||
        price <= 0 ||
        deposit < 0 ||
      ...
      KnzToast.warning(
        context,
        'Deposit must be between zero and the agreed price.',
      );
      return;
    }
    ...
    try {
      if (_isEdit) {
        ...
        if (mounted) {
          ...
          KnzToast.success(
            context,
            '✏️ Custom order for "$customerName" updated.',
          );
        }
      } else {
        final customerName = _nameCtrl.text.trim();
        ...
        if (mounted) {
          ...
          KnzToast.success(
            context,
            '✅ Custom order for "$customerName" created.',
          );
        }
      }
    } catch (_) {
      if (mounted) {
        KnzToast.error(
          context,
          'The custom order could not be saved. Please try again.',
        );
      }
    } finally {
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Text(
                    _isEdit ? 'Edit Agreement' : 'New Custom Order',
                ...
                children: [
                  Expanded(
                    child: _field(
                      ...
                      validator: (v) {
                        final n = Money.tryParse(v ?? '');
                        if (n == null || !n.isPositive) return 'Enter price';
                        return null;
                      },
                    ),
                  ),
                  ...
                  Expanded(
                    child: _field(
                      _depositCtrl,
                      _isEdit ? 'Deposit Paid (recorded)' : 'Deposit Paid (₱)',
                      '0.00',
                      readOnly: _isEdit,
                      keyboardType: const TextInputType.numberWithOptions(
                        decimal: true,
                      ...
                      validator: (v) {
                        final n = Money.tryParse(v ?? '');
                        final price = Money.tryParse(_priceCtrl.text.trim());
                        if (n == null || n.isNegative) return 'Enter deposit';
                        if (price != null && n > price) return 'Above price';
                        return null;
                      },
                    ),
                  ),
              ...
              _field(
                ...
                'e.g. No refund after production starts…',
                maxLines: 3,
              ),
