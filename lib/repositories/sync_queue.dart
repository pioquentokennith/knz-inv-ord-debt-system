...
class SyncQueue implements SyncOutbox {
  ...
  Future<void> startMonitoring() async {
    ...
    _connectivitySub = Connectivity().onConnectivityChanged.listen(
      _handleConnectivity,
      onError: (Object _, StackTrace __) => stopMonitoring(),
    );
  ...
  Future<SyncProcessResult> syncPending() async {
    if (_isSyncing || !_isOnline) return const SyncProcessResult.skipped();
  ...
  Future<void> _emitStatus(String userId) async {
    _statusController.add(await statusForUser(userId));
  }
