...
class LocalOrderRepository extends BaseRepository implements OrderRepository {
  ...
  Future<Order> addWithInventory(
    ...
    if (order.status == OrderStatus.pending && debt != null) {
      throw const InvalidOrderTransitionException(
        'A Pending order cannot start with an automatic debt ledger.',
      );
    }
    ...
    await database.transaction((txn) async {
      ...
      if (debt != null) {
        await _insertDebt(txn, debt, savedOrder.orderId, userId);
      }
      await _queueOrderUpsert(txn, savedOrder, userId);
  ...
  Future<Order> _insertOrder(
    ...
    final dto = OrderDto.fromDomain(saved, userId: userId);
  ...
  Future<void> _queueOrderUpsert(
    DatabaseExecutor txn,
    Order order,
    String userId,
  ) async {
    final dto = OrderDto.fromDomain(order, userId: userId);
    await _queue.enqueue(
      operation: 'save_order',
      collection: 'orders',
      userId: userId,
  ...
  Future<void> _adjustInventory(
    ...
    for (final entry in quantities.entries) {
      ...
      await _queue.enqueue(
        ...
        userId: userId,
        docId: entry.key,
        data: ProductDto.fromLocal(row).toCloud(),
        executor: txn,
      );
    }
  }
